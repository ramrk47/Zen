...
def _fill_names_from_ids(payload_dict: dict, db: Session) -> dict:
    ...
    if branch_id is not None:
        ...
        if bank_id is not None and branch.bank_id != bank_id:
            raise HTTPException(
                status_code=400, detail="branch_id does not belong to bank_id"
            )
...
def _validate_by_case_type(case_type: str, data: dict):
    ...
    if ct == "BANK":
        if data.get("bank_id") is None and not data.get("bank_name"):
            raise HTTPException(
                status_code=400, detail="BANK case requires bank_id (or bank_name)"
            )
        if data.get("branch_id") is None and not data.get("branch_name"):
            raise HTTPException(
                status_code=400, detail="BANK case requires branch_id (or branch_name)"
            )
...
def create_assignment(
    ...
    # Backend-enforced security: employees cannot set money fields
...
def update_assignment(
    ...
    # Backend-enforced security: employees cannot change money fields
    if not _is_admin(current_user):
...
def delete_assignment(
    ...
    # Optional: only admin delete (uncomment if you want strict)
    # if not _is_admin(current_user):
